...
## Why a rate meter reads under 1000 Hz

Three different things get called "polling rate":

1. **The endpoint's slot rate** (what "1000 Hz" means on the spec sheet):
   the host offers the device a transfer slot every 1 ms. This is what
   determines added latency — a change is on the wire within the next slot.
2. **What any report-timestamp meter can see** (this tool, or the reader's
   readout): HID delivers a report **only when its content changes**. A
   stick held at a rail, a reversal, a pause between button presses —
   every quiet millisecond produces no report and drags the *average*
   down. The **median ("sustained") interval** is the honest number: with
   fast, continuous, never-stopping stick circles it sits at ~1 ms
   (≈1000 Hz) on this pad. An average under 1000 does not mean the pad is
   slow — it means your hand paused.
3. **XInput / browser meters** (gamepad-tester sites, `joypad-bench.html`,
   games, anything built on `XInputGetState` or the browser Gamepad API):
   these read through the 125 Hz host-side path this project bypasses.
   They will show ~125 Hz (or the display refresh) forever, no matter
   what the pad or the raw reader does. Only the raw IG_00 path gets the
   fast rate, and only consumers of that path (ReflexX's provider) see it.

To verify the true endpoint rate: `dotnet run --` and spin one stick in
fast continuous circles for the whole window without ever stopping —
median ≈1.0 ms, modal histogram bucket at 1 ms.
