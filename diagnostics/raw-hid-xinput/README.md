...
Its usings and namespace now match the real tree — `ReflexX.Domain`,
`.Entities`, `.Enums`, `.Interfaces`, `.ValueObjects`, per
`Rp2350AppTransport` — and it carries the same member surface
(`RestartAsync`, `PulseRumble` no-op, `ExcludeXInputSlots`, …), so it should
drop in with **no edits**. The only fixes that may remain:

1. `GamepadButton` member names in `ButtonMap` and `s_hat8` (A/B/X/Y,
   shoulders, thumbs, Back/Start, dpad) — rename if the real enum spells
   them differently. The *order* is confirmed from the pad (press order
   `1..10` = A B X Y LB RB Back Start L3 R3).
2. Any `IInputProvider` members beyond these → stub them like
   `ExcludeXInputSlots`.
