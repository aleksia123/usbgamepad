...
public sealed class RawHidXInputProvider : IInputProvider, IDisposable
{
    ...
    /// call (plan Step 3 - surface via the Logs panel or poll ~1/s).</summary>
    public void LogDiagnostics(string label)
    {
        ...
        _logger.LogInformation(
            "Raw HID pad ({Label}): {Status}; {Count} reports, min={Min:F3} avg={Avg:F3} max={Max:F3} ms (~{Hz:F0} Hz)",
            label, IsConnected ? "connected" : "disconnected", count, min, avg, max,
            avg > 0 ? 1000.0 / avg : 0);
    }
