...
/// <summary>Snapshot of achieved report timing since the previous snapshot.
/// HID reports are delivered ON CHANGE only, so pauses in input stretch the
/// average; the MEDIAN reflects the endpoint's sustained cadence while input
/// is actually moving and is the number to compare against 1000 Hz.</summary>
public sealed class RateStats
{
    public int Samples;
    public double MinMs, MedianMs, AvgMs, MaxMs, LastMs;
    public long TotalReports;

    public override string ToString() => Samples == 0
        ? "no reports"
        : $"{Samples} reports  min={MinMs:F3}  med={MedianMs:F3}  avg={AvgMs:F3}  max={MaxMs:F3} ms  " +
          $"(~{1000.0 / Math.Max(MedianMs, 1e-9):F0} Hz sustained; avg includes input pauses)";
}
...
public sealed class RawHidGamepadReader : IDisposable
{
    ...
    private long _total;
    private readonly List<double> _intervalSamples = new();
    ...
    public RateStats GetRateStats()
    {
        lock (_statsLock)
        {
            var s = new RateStats
            {
                Samples = _cnt,
                MinMs = _cnt > 0 ? _min : 0,
                MedianMs = MedianOf(_intervalSamples),
                    ...
                    if (havePrev)
                    {
                        ...
                        if (ms <= 250) // longer = input pause, not a polling interval
                        {
                            lock (_statsLock)
                            {
                                ...
                                _lastMs = ms;
                                _total++;
                                if (_intervalSamples.Count < 8192) _intervalSamples.Add(ms);
                            }
                        }
                    }
                    prevT = t;
