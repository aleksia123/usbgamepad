...
bool xid_get_report(uint8_t index, void *report, uint16_t len)
{
    ...
    if (tud_ready() && !usbd_edpt_busy(TUSB_GAMEPAD_RHPORT, _xid_itf[index].ep_out))
    {
        usbd_edpt_xfer(TUSB_GAMEPAD_RHPORT, _xid_itf[index].ep_out, _xid_itf[index].ep_out_buff, len);
    }
    return true;
}
...
bool xid_send_report_ready(uint8_t index)
{
    ...
    return (tud_ready() && !usbd_edpt_busy(TUSB_GAMEPAD_RHPORT, _xid_itf[index].ep_in));
}
bool xid_send_report(uint8_t index, void *report, uint16_t len)
{
    ...
    //Send it to the host
    return usbd_edpt_xfer(TUSB_GAMEPAD_RHPORT, _xid_itf[index].ep_in, _xid_itf[index].in, len);
}
