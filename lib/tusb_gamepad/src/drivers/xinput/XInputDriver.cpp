...
XInputOutReport xinput_out_report;
...
static bool xinput_xfer_callback(uint8_t rhport, uint8_t ep_addr, xfer_result_t result, uint32_t xferred_bytes)
{
	...
	memcpy(&xinput_out_report, &xinput_out_buffer, sizeof(XInputOutReport));
...
void XInputDriver::update_rumble(int idx, Gamepad * gamepad)
{
	...
	{
		gamepad->rumble.l = xinput_out_report.lrumble;
		gamepad->rumble.r = xinput_out_report.rrumble;
	}
}